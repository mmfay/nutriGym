	...
	return (
		<div className="space-y-6">
			...
			<div className="flex gap-2">
				...
				<button
					onClick={() => { setMode("edit"); mc.fetchRecipes(); }}
					className={[
						"px-3 py-1.5 rounded border text-sm",
						mode === "edit"
							? "bg-gray-900 text-white dark:bg-gray-100 dark:text-gray-900"
...
function SavedMeals({ mc }: { mc: ReturnType<typeof useRecipeController> }) {

	if (mc.loading) {
		return <p className="text-sm text-gray-500 dark:text-gray-400">Loading…</p>;
	}

	if (mc.recipes.length === 0) {
		return <p className="text-sm text-gray-500 dark:text-gray-400 italic">No saved meals yet. Create one in the Create tab.</p>;
	}

	return (
		<div className="space-y-4">
			{mc.recipes.map((recipe) => {
				...
				return (
					...
					>
						...
						<div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-700">
							...
							<button
								onClick={() => mc.deleteSavedRecipe(recipe.id)}
